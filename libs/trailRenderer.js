...
THREE.TrailRenderer.createMaterial = function (
  vertexShader,
  fragmentShader,
  customUniforms
) {
  ...
  fragmentShader =
    fragmentShader || THREE.TrailRenderer.Shader.BaseFragmentShader;
...
THREE.TrailRenderer.prototype.initialize = function (
  material,
  length,
  dragTexture,
  localHeadWidth,
...
THREE.TrailRenderer.prototype.initializeLocalHeadGeometry = function (
  localHeadWidth,
  localHeadGeometry
) {
  ...
  if (!localHeadGeometry) {
    ...
    for (
      var i = 0;
      i < localHeadGeometry.length && i < THREE.TrailRenderer.MaxHeadVertices;
      i++
    ) {
...
THREE.TrailRenderer.prototype.initializeGeometry = function () {
  ...
  var positions = new Float32Array(
    this.vertexCount * THREE.TrailRenderer.PositionComponentCount
  );
  var nodeCenters = new Float32Array(
    this.vertexCount * THREE.TrailRenderer.PositionComponentCount
  );
  var uvs = new Float32Array(
    this.vertexCount * THREE.TrailRenderer.UVComponentCount
  );
  var indices = new Uint32Array(
    this.faceCount * THREE.TrailRenderer.IndicesPerFace
  );
...
THREE.TrailRenderer.prototype.advanceWithPositionAndOrientation = function (
  nextPosition,
  orientationTangent
) {
  this.advanceGeometry(
    { position: nextPosition, tangent: orientationTangent },
    null
  );
};

THREE.TrailRenderer.prototype.advanceWithTransform = function (
  transformMatrix
) {
...
THREE.TrailRenderer.prototype.advanceGeometry = (function () {
  ...
  return function advanceGeometry(positionAndOrientation, transformMatrix) {
    var nextIndex =
      this.currentEnd + 1 >= this.length ? 0 : this.currentEnd + 1;
...
THREE.TrailRenderer.prototype.updateNodeCenter = function (
  nodeIndex,
  nodeCenter
) {
  ...
  nodeCenters.updateRange.offset =
    nodeIndex *
    this.VerticesPerNode *
    THREE.TrailRenderer.PositionComponentCount;
  nodeCenters.updateRange.count =
    this.VerticesPerNode * THREE.TrailRenderer.PositionComponentCount;
};
...
THREE.TrailRenderer.prototype.updateNodePositionsFromOrientationTangent = (function () {
  ...
  return function updateNodePositionsFromOrientationTangent(
    nodeIndex,
    nodeCenter,
    orientationTangent
  ) {
    ...
    tempQuaternion.setFromUnitVectors(
      THREE.TrailRenderer.LocalOrientationTangent,
      orientationTangent
    );
    ...
    for (var i = 0; i < this.localHeadGeometry.length; i++) {
      var positionIndex =
        (this.VerticesPerNode * nodeIndex + i) *
        THREE.TrailRenderer.PositionComponentCount;
...
THREE.TrailRenderer.prototype.updateNodePositionsFromTransformMatrix = (function () {
  ...
  return function updateNodePositionsFromTransformMatrix(
    nodeIndex,
    transformMatrix
  ) {
    var positions = this.geometry.getAttribute("position");
    ...
    if (this.lastNodeCenter && this.orientToMovement) {
      ...
      if (tempDirection.lengthSq() > 0.0001) {
        if (!this.lastOrientationDir)
          this.lastOrientationDir = new THREE.Vector3();
    ...
    positions.updateRange.offset =
      nodeIndex *
      this.VerticesPerNode *
      THREE.TrailRenderer.PositionComponentCount;
    positions.updateRange.count =
      this.VerticesPerNode * THREE.TrailRenderer.PositionComponentCount;
  };
})();
...
THREE.TrailRenderer.prototype.connectNodes = (function () {
  ...
  return function connectNodes(srcNodeIndex, destNodeIndex) {
    ...
    for (var i = 0; i < this.localHeadGeometry.length - 1; i++) {
      ...
      var faceIndex =
        (srcNodeIndex * this.FacesPerNode +
          i * THREE.TrailRenderer.FacesPerQuad) *
        THREE.TrailRenderer.IndicesPerFace;
    ...
    returnObj.offset =
      srcNodeIndex * this.FacesPerNode * THREE.TrailRenderer.IndicesPerFace;
